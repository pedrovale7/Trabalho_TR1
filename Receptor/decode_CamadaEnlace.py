...
def verifica_bit_parity(binary_sequence: list[int]):
    ...
    if parity !=0:
        raise Exception("Erro detectado - Bit de Paridade")
    return binary_sequence[:-1]                                 # Remove o ultimo bit (paridade)
...
def verifica_crc(data_recebida, polinomio):
    ...
    resto = ce.crc_checksum(data_recebida, polinomio)
    if not (all(bit == '0' for bit in resto)):
        raise Exception("Erro detectado - CRC")

def corr_haming(binary_sequence):                         # Função que faz a correção do erro
